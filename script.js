(() => {
  ...
  const modifierLabels = { none: "Aucun", shift: "Maj", altgr: "Alt Gr" };
  const baseLabels = { "0": "0", e: "E", ",": ",", ":": ":", a: "A" };
  ...
  function loadSymbolTask() {
    ...
    symbolHintText.textContent = task.modifier === "none" ? "Une seule touche suffit." : "Deux touches sont nécessaires.";
    comboResult.textContent = "…";
    document.querySelectorAll("#modifierChoices button").forEach(button => button.classList.remove("is-selected"));
  }
  ...
  document.getElementById("symbolKeys").addEventListener("click", event => {
    ...
    if (state.selectedModifier === null) {
      ...
      setFeedback(symbolFeedback, "Commence par choisir <strong>Aucun</strong>, <strong>Maj</strong> ou <strong>Alt Gr</strong>.", "error");
      return;
    }
    const task = symbolTasks[state.symbolIndex];
    const base = button.dataset.base;
    comboResult.textContent = `${modifierLabels[state.selectedModifier]} + ${baseLabels[base]}`;
  ...
  const helpContent = {
    ...
    symbols: {
      title: "Lire les trois niveaux d’une touche",
      html: `<ul><li>Le signe principal s’écrit seul.</li><li>Le signe du haut s’obtient avec <kbd>Maj</kbd>.</li><li>Le troisième signe, souvent en bas à droite, s’obtient avec <kbd>Alt Gr</kbd>.</li></ul><div class="key-demo"><kbd>Alt Gr</kbd><span>+</span><kbd>0 / à / @</kbd><span>→ @</span></div><div class="key-demo"><kbd>Alt Gr</kbd><span>+</span><kbd>E / €</kbd><span>→ €</span></div>`
    },
    shortcuts: {
